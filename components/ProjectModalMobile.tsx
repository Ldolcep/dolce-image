// ========================================================================
// === ProjectModalMobile.tsx - CORRECTION setNextImageStateIndex ===
// ========================================================================
"use client"
...
export default function ProjectModalMobile({ project, isOpen, onClose }: ProjectModalMobileProps) {
  ...
  // L'état nextImageStateIndex et son useEffect de mise à jour sont supprimés.
  // Nous utilisons swipeBackgroundInfo.index directement pour le rendu de l'image de fond.
  ...
  const calculateSwipeAnimation = useCallback((currentX: number, startX: number) => {
      ...
      try {
        ...
        setSwipeBackgroundInfo({ index: bgIndex, direction: swipeDir }); // Ceci déclenchera un re-render avec la bonne image de fond
        ...
        requestAnimationFrame(() => {
            ...
            if (bgIndex !== null) { // Si une image de fond doit être montrée
                ...
                backgroundImageRef.current.style.zIndex = '5';
            } else { // Sinon, s'assurer qu'elle est cachée
                backgroundImageRef.current.style.opacity = '0';
            }
       });
    } catch (error) { console.error("Swipe calc error:", error); }
  }, [isSwiping, isMounted, currentImageIndex, allVisuals.length, nextIndex, prevIndex]); // prevIndex et nextIndex sont les constantes useMemo

  const onTouchStart = useCallback((e: React.TouchEvent) => { /* (Logique comme avant, inchangée) */ }, [allVisuals.length, isDraggingPanel, isInfoVisible]);
  const onTouchMove = useCallback((e: React.TouchEvent) => { /* (Logique comme avant, inchangée) */ }, [touchStart, isSwiping, currentTouchX, calculateSwipeAnimation, resetSwipeState, isDraggingPanel]);
  const onTouchEnd = useCallback(() => { /* (Logique comme avant, inchangée, dépend de swipeBackgroundInfo) */ }, [ touchStart, touchEnd, isSwiping, swipeRotation, MIN_SWIPE_DISTANCE, handleNext, handlePrevious, resetSwipeState, swipeBackgroundInfo ]);
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        // setNextImageStateIndex(allVisuals.length > 1 ? 1 : 0); // Supprimé car géré par swipeBackgroundInfo et l'effet sur nextIndex
  ...
  useEffect(() => {
    ...
    if (isOpen && isMounted) { timeoutId = setTimeout(() => {}, 50); /* setIsAnimating retiré, non utilisé */ }
    // else { setIsAnimating(false); } // setIsAnimating retiré, non utilisé
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 overflow-hidden select-none" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby={isInfoVisible ? undefined : `modal-title-${project.id}`}>
        ...
        <div className="absolute inset-0 pt-16 pb-[--grip-visible-height] overflow-hidden" style={{ '--grip-visible-height': collapsedGripVisibleHeight } as React.CSSProperties} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            {/* Background Image utilise maintenant swipeBackgroundInfo.index */}
