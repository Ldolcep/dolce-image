// ProjectModalMobile.tsx - CORRIGÉ ET SIMPLIFIÉ
"use client"
...
// Constants
export const GRIP_HEIGHT_COLLAPSED = '3vh'; // Valeur que vous avez définie
...
export default function ProjectModalMobile({ project, isOpen, onClose }: ProjectModalMobileProps) {
  ...
  // isTransitioning et nextImageStateIndex sont supprimés

  const modalRef = useRef<HTMLDivElement>(null);
  const imageRef = useRef<HTMLDivElement>(null);
  const backgroundImageRef = useRef<HTMLDivElement>(null); // Renommé pour clarté (était nextImageRef)
  ...
  // useEffect pour nextImageStateIndex est supprimé
  ...
  const resetSwipeState = useCallback(() => {
    ...
    requestAnimationFrame(() => {
        try {
            ...
            if (backgroundImageRef.current) { // Utilise backgroundImageRef
  ...
  }, []); // Pas de dépendance à isTransitioning
  ...
  // handleSwipedNext et handleSwipedPrevious sont supprimées

  const calculateSwipeAnimation = useCallback((currentX: number, startX: number) => {
      if (!isMounted || !isSwiping || !imageRef.current || !backgroundImageRef.current) return; // Utilise backgroundImageRef
      try {
        ...
        requestAnimationFrame(() => {
            if (!imageRef.current || !backgroundImageRef.current) return; // Utilise backgroundImageRef
  ...
  const onTouchStart = useCallback((e: React.TouchEvent) => {
    // La logique de vérification du panneau pour le scroll est gardée
    ...
    if (allVisuals.length <= 1 || isDraggingPanel || isSwiping) return; // Ajout de isSwiping ici
    ...
    try {
        ...
        if (backgroundImageRef.current) backgroundImageRef.current.style.transition = 'none'; // Utilise backgroundImageRef
    } catch (error) { console.error("TouchStart error:", error); setIsSwiping(false); }
  }, [allVisuals.length, isDraggingPanel, isInfoVisible, isSwiping]); // isSwiping ajouté aux deps
 ...
 const onTouchEnd = useCallback(() => {
    ...
    // Important: Mettre isSwiping à false ici, avant les setTimeout et la logique d'animation
    // pour éviter que de nouveaux touchStart ne soient bloqués par un isSwiping qui resterait true trop longtemps.
    setIsSwiping(false);
    ...
    try {
        ...
        if (isValidLeftSwipe || isValidRightSwipe) {
            ...
            if (backgroundImageRef.current && swipeBackgroundInfo.index !== null) { // Utilise backgroundImageRef
        ...
        if (isValidLeftSwipe) {
            ...
            setTimeout(() => {
                handleNext(); // Appelle handleNext (et non handleSwipedNext)
                resetSwipeState();
            }, swipeAnimDuration);
            ...
            if(backgroundImageRef.current) { backgroundImageRef.current.style.transform = 'scale(1) translateY(0px)'; backgroundImageRef.current.style.opacity = '1'; backgroundImageRef.current.style.zIndex = '15'; }
            setTimeout(() => {
                handlePrevious(); // Appelle handlePrevious (et non handleSwipedPrevious)
                resetSwipeState();
            }, swipeAnimDuration);
        } else {
            resetSwipeState();
        ...
        setTouchStart(null); setTouchEnd(null); setCurrentTouchX(null);
        // isSwiping est déjà false, swipeBackgroundInfo est géré par resetSwipeState
    }
  }, [ touchStart, touchEnd, isSwiping, swipeRotation, MIN_SWIPE_DISTANCE, handleNext, handlePrevious, resetSwipeState, swipeBackgroundInfo ]); // handleSwipedNext/Previous retirés

  const calculatePanelY = useCallback((deltaY: number): number => { /* (Logique comme avant) */ }, [isMounted]);
  const updateContentVisibility = useCallback((currentY: number | null) => { /* (Logique comme avant) */ }, []);
  const handlePanelTouchStart = useCallback((e: React.TouchEvent) => { /* (Logique comme avant) */ }, [isInfoVisible, isSwiping]);
  const handlePanelTouchMove = useCallback((e: React.TouchEvent) => { /* (Logique comme avant) */ }, [dragStartY, isDraggingPanel, calculatePanelY, updateContentVisibility]);
  const handlePanelTouchEnd = useCallback(() => { /* (Logique comme avant) */ }, [dragStartY, isDraggingPanel, panelTranslateY, isInfoVisible]);

  // --- EFFECTS ---
  useEffect(() => {
    if (isOpen && isMounted && initialCollapsedY !== null) {
        setPanelTranslateY(initialCollapsedY);
        if (panelRef.current) { // Vérification ajoutée
            panelRef.current.style.transform = `translateY(${initialCollapsedY}px)`; // Appliquer transform initial
            panelRef.current.style.visibility = 'visible'; // S'assurer qu'il est visible
            requestAnimationFrame(() => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        // nextImageStateIndex est supprimé
    ...
    <div className="fixed inset-0 bg-white z-50 overflow-hidden select-none" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby={isInfoVisible ? undefined : `modal-title-${project.id}`}>
        ...
        <div ref={panelRef}
             ...
             style={{
                 ...
                 zIndex: 30,
                 // Visibilité initiale pour s'assurer qu'il est dans le layout pour les calculs
        ...
        >
            <div ref={gripRef} className="w-full flex flex-col items-center pt-3 pb-2 pointer-events-none">
                ...
                <div className="flex-grow flex items-center justify-center w-full px-4 overflow-hidden" style={{minHeight: `calc(${GRIP_HEIGHT_COLLAPSED} - 30px)`}} >
