// ProjectModalMobile.tsx - CORRECTIONS POUR SWIPE DYNAMIQUE (Basé sur V0.22)
"use client"
...
// Constants
export const GRIP_HEIGHT_COLLAPSED = '3vh'; // Votre valeur de V0.22
...
export default function ProjectModalMobile({ project, isOpen, onClose }: ProjectModalMobileProps) {
  ...
  // Supprimé : const [nextImageStateIndex, setNextImageStateIndex] = useState(allVisuals.length > 1 ? 1 : 0);

  // NOUVEL ÉTAT pour l'image d'arrière-plan pendant le swipe
  ...
  // Supprimé : const [isTransitioning, setIsTransitioning] = useState(false);

  const modalRef = useRef<HTMLDivElement>(null);
  const imageRef = useRef<HTMLDivElement>(null);
  const backgroundImageRef = useRef<HTMLDivElement>(null); // Renommé (était nextImageRef dans V0.22)
  ...
  // Supprimé : useEffect pour setNextImageStateIndex

  const resetSwipeState = useCallback(() => {
    // Supprimé : if (isTransitioning) return;
    ...
    requestAnimationFrame(() => {
        try {
            ...
            if (backgroundImageRef.current) { // Utilise backgroundImageRef
                ...
                backgroundImageRef.current.style.transition = 'none'; // Reset immédiat
                backgroundImageRef.current.style.zIndex = '5';
            }
        } catch (error) { console.error("Swipe reset style error:", error); }
    });
  }, []); // isTransitioning retiré des dépendances
  ...
  // Supprimé : handleSwipedNext et handleSwipedPrevious
 ...
 const onTouchEnd = useCallback(() => {
  ...
  const calculatePanelY = useCallback((deltaY: number): number => { /* (Logique inchangée) */ }, [isMounted]);
  const updateContentVisibility = useCallback((currentY: number | null) => { /* (Logique inchangée) */ }, []);
  const handlePanelTouchStart = useCallback((e: React.TouchEvent) => { /* (Logique inchangée) */ }, [isInfoVisible, isSwiping]);
  const handlePanelTouchMove = useCallback((e: React.TouchEvent) => { /* (Logique inchangée) */ }, [dragStartY, isDraggingPanel, calculatePanelY, updateContentVisibility]);
  const handlePanelTouchEnd = useCallback(() => { /* (Logique inchangée) */ }, [dragStartY, isDraggingPanel, panelTranslateY, isInfoVisible]);
  ...
  // Supprimé : useEffect pour isTransitioning
