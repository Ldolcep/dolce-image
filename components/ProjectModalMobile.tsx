// ProjectModalMobile.tsx - VERSION CORRIGÉE ET COMPLÉTÉE
"use client"
...
// Constants
export const GRIP_HEIGHT_COLLAPSED = '2.5vh'; // Mise à jour ici comme demandé
...
export default function ProjectModalMobile({ project, isOpen, onClose }: ProjectModalMobileProps) {
  ...
  const backgroundImageRef = useRef<HTMLDivElement>(null); // RENOMMÉ (était nextImageRef)
  ...
  const initialCollapsedY = useMemo(() => {
    if (typeof window !== 'undefined' && isMounted) {
      try {
        ...
        const gripHeightPx = vh * parseFloat(GRIP_HEIGHT_COLLAPSED) / 100; // Utilise la constante correcte
  ...
  // Pas besoin de nextImageStateIndex et son useEffect si swipeBackgroundInfo gère l'image de fond pour le swipe
  ...
  const onTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    if (allVisuals.length <= 1 || isDraggingPanel || isSwiping) { return; } // isSwiping ajouté ici
    setIsSwiping(true);
    try {
 ...
 const onTouchEnd = useCallback(() => {
    if (!touchStart || !isSwiping) { if (!isSwiping && touchStart !== null) resetSwipeState(); return; } // Si touchStart a eu lieu mais pas isSwiping, reset.
    ...
    setIsSwiping(false); // Important de le mettre ici
    try {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        // L'état nextImageStateIndex n'est plus directement défini ici,
        // il sera mis à jour par l'effet dépendant de nextIndex (qui dépend de currentImageIndex)
  ...
  const collapsedGripVisibleHeight = `calc(${GRIP_HEIGHT_COLLAPSED} + 0px)`; // Utilise la constante correcte
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 overflow-hidden select-none" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby={isInfoVisible ? undefined : `modal-title-${project.id}`}>
        ...
        <div className="absolute inset-0 pt-16 pb-[--grip-visible-height] overflow-hidden" style={{ '--grip-visible-height': collapsedGripVisibleHeight } as React.CSSProperties} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            {/* Background Image utilise swipeBackgroundInfo.index */}
